...
Let's consider sampled data from Feb. 2022 -- which starting station is the most popular one?
```{r}
...
Where are the available positions in Feb. 2022?
```{r}
pal <- colorNumeric(
  palette = "Purples",
  domain = station_data$total_docks
)
...
BostonMap <- leaflet(data = station_data) %>% 
  ...
  addCircleMarkers(lng = station_data$longitude, 
                   ...
                   radius = 1.5, 
                   color = ~pal(total_docks),
                   popup = paste0("Station: ", station_data$name, "<br>",
                             "# docks: ", station_data$total_docks),)

BostonMap
```

What if I want to see the trip data from the Jan. 2015?
```{r}
quarter_1_2021 <- import_month_data(2015, 1)
```
